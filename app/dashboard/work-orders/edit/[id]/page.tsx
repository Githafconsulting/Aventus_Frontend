...
export default function EditWorkOrderPage() {
  ...
  // Fetch work order and related data on mount
  useEffect(() => {
    if (workOrderId) {
      fetchWorkOrder();
      fetchContractors();
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          actual_hours: data.actual_hours?.toString() || "0",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.title) workOrderData.title = formData.title;
      if (formData.description) workOrderData.description = formData.description;
      if (formData.contractor_id) workOrderData.contractor_id = formData.contractor_id;
      ...
      if (formData.start_date) workOrderData.start_date = new Date(formData.start_date).toISOString();
      ...
      if (formData.actual_hours) workOrderData.actual_hours = parseFloat(formData.actual_hours);
      if (formData.status) workOrderData.status = formData.status;
